...
function App() {
  ...
  const addToWhiteList = async () => {
    ...
    if (isConnected) {
      ...
      const monarchAppSocietyContract = new ethers.Contract(
        monarchAppSocietyContractAddress,
        cyberShareABI,
        provider.getSigner()
      );

      await monarchAppSocietyContract.whitelistUser(addAddress.current.value);
    } else {
      connect();
    }
  };
  const removeFromWhiteList = async () => {
    ...
    if (isConnected) {
      ...
      await monarchAppSocietyContract.removeWhitelistUser(
        removeAddress.current.value
      );
    } else {
      connect();
    }
  ...
  const checkIsWhiteListed = async () => {
    ...
    if (isConnected) {
      ...
      let res = await monarchAppSocietyContract.whitelisted(
  ...
  // );
  const Msg = ({ closeToast, toastProps, hash }) => (
    <div className="text-white bg-[#A07834]">
      Token ${totalSupply + 1} Minted to {account} <br /> Block URI:
      <a
        className="text-underline  text-[#A07834] text-bold"
        href={`https://rinkeby.etherscan.io/tx/${hash}`}
        target="_blank"
      >
  ...
  const ErrorMsg = ({ closeToast, toastProps, e }) => (
    <div className="mx-auto text-[#A07834] text-bold ">
  ...
  // mainnet
  const monarchAppSocietyContractAddress =
    "0xB916fD414Bf8EB99435550A2aeb4FDf4ACBA2EE7";
  // Polygon
  // const monarchAppSocietyContractAddress =
  ...
  // Rinkiby
  // const monarchAppSocietyContractAddress =
  //   "0x85831CA94a390Df62f8008B6976dDD89c975E25E";
  // const monarchAppSocietyContractAddress =
  //   "0xdF6110841d03EA58de3C89E0a29a16B23b0c7d00"; owner
  // const monarchAppSocietyContractAddress =
  ...
  useEffect(() => {
    async function a() {
      ...
      if (window.ethereum) {
        ...
        // await monarchAppSocietyContract.mint(
        ...
        let owner1 = BigNumber.from(await monarchAppSocietyContract.owner());
        ...
        setMaxMint(
          BigNumber.from(await monarchAppSocietyContract.maxMint()).toNumber()
        );
        setTotalSupply(
          BigNumber.from(
            await monarchAppSocietyContract.totalSupply()
          ).toNumber()
        );
        // setAccount1(BigNumber.from(await monarchAppSocietyContract.ownerOf(count)));
        setIsPreSale(await monarchAppSocietyContract.preSale());
        setIsRevealed(await monarchAppSocietyContract.reveal());
        // console.log(await monarchAppSocietyContract.owner);
  ...
  const transferNFT = async (monarchAppSocietyContract) => {
    // setCount(BigNumber.from(await monarchAppSocietyContract.count()).toNumber());
    // setAccount1(BigNumber.from(await monarchAppSocietyContract.ownerOf(count)));
    // var arr = [account];
    // console.log(account1._hex);
    ...
    // let countrrr = [count];
    const checkIsPreSale = await monarchAppSocietyContract.preSale();
    let options;
    let c;
    if (!isAdmin) {
      if (checkIsPreSale) {
        options = { value: await monarchAppSocietyContract.preSaleCost() };
        let check = await monarchAppSocietyContract.whitelisted(account);
        // console.log(check);
        if (!check) {
        ...
        options = { value: await monarchAppSocietyContract.cost() };
      }
      try {
        c = await monarchAppSocietyContract.mint(1, options);
      ...
      try {
        c = await monarchAppSocietyContract.mint(1);
        setTotalSupply(totalSupply + 1);
      } catch (e) {
        notifyError(e.message.split(" ").slice(0, 5).join(" "));
        // console.log("Error:", e.message.split(" ").slice(0, 10).join(" "));
    ...
    //   await monarchAppSocietyContract.setCount(count + 1);
    //   await transferNFT(monarchAppSocietyContract);
    // }
  };
  ...
  const reveal = async () => {
    if (isConnected) {
      ...
      try {
        await monarchAppSocietyContract.toggleReveal();
  ...
  const preSale = async () => {
    if (isConnected) {
      ...
      try {
        await monarchAppSocietyContract.togglePresale();
  ...
  const withdraw = async () => {
    if (isConnected) {
      ...
      try {
        const bal = await monarchAppSocietyContract.getContractBalance();
        console.log(bal);
        if (bal <= 0) {
          // console.error("0 balance");
          notifyError("Zero Balance");
          return;
        }
        await monarchAppSocietyContract.withdraw();
        // setIsPreSale(!isPreSale);
      } catch (e) {
        notifyError(e.message.split(" ").slice(0, 5).join(" "));
        // console.log("Error:", e.message.split(" ").slice(0, 10).join(" "));
